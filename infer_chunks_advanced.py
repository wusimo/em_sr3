...
def prepare_input(input_map_file_path, temp_input_dir):
    ...
    # Using the same parameters as in eval.py
    block_size = 32  # Adjust based on your model's input size
    stride_size = 24  # Adjust based on your needs
    
    logger.info(f"Extracting patches with block_size={block_size}, stride_size={stride_size}")
    
    # Use EMPatches to extract blocks and indices
...
def merge_output(temp_output_dir, output_map_file_path, temp_input_dir, input_map_file_path):
    """Merge processed chunks back into a single MRC file using EMPatches"""
    logger = logging.getLogger('base')
    ...
    if len(prediction_files) == 0:
        logger.error(f"No prediction files found in {temp_output_dir}")
        return False
    
    # Load original map to get dimensions
    with mrcfile.open(input_map_file_path) as mrc:
    ...
    for i, pred_file in enumerate(prediction_files):
        ...
        preds[pred_idx] = pred
        indices[pred_idx] = block_indices[pred_idx]
