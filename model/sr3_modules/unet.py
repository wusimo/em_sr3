...
class SelfAttention(nn.Module):
    ...
    def forward(self, input):
        batch, channel, depth, height, width = input.shape  # Adjusted shape for 3D
        n_head = self.n_head
        head_dim = channel // n_head

        norm = self.norm(input)
        qkv = self.qkv(norm).view(batch, n_head, head_dim * 3, depth, height, width)
        query, key, value = qkv.chunk(3, dim=2)  # bhdxyz

        attn = torch.einsum(
            "bncdhw, bncyxz -> bndhwyxz", query, key
        ...
        out = torch.einsum("bndhwyxz, bncyxz -> bncdhw", attn, value).contiguous()
